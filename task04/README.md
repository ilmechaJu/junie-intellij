# 고급 프로젝트 개발 가이드: Junie를 활용한 도서관 관리 시스템 구축

이 가이드는 MVC 패턴, JUnit 테스트, JDBC, 데이터베이스 연결 등을 활용하여 도서관 관리 시스템을 구축하는 방법을 안내합니다. Junie의 강력한 코드 생성 및 프로젝트 구성 기능을 활용하여 체계적이고 전문적인 프로젝트를 개발해 보세요.
...
7. [테스트 및 배포](#테스트-및-배포)
8. [추가 기능 및 확장](#추가-기능-및-확장)

## 프로젝트 소개

도서관 관리 시스템은 다음과 같은 기능을 제공하는 자바 애플리케이션입니다:
...
- 연체 관리 및 알림
- 도서 예약 시스템
- 보고서 생성 (인기 도서, 연체 현황 등)

이 프로젝트는 MVC 아키텍처 패턴을 적용하여 코드의 유지보수성과 확장성을 높이고, 데이터베이스 연동을 통해 실제 운영 환경에서 활용 가능한 수준의 애플리케이션을 개발하는 것을 목표로 합니다.

## 사용방법
1. `3.시스템 요구사항` 이하의 모든 내용을 복사해서, ./task04/guidelines.md 안에 내용을 붙여넣기 합니다.
![img_1.png](img_1.png)
2. Junie를 `Code`모드로 선택 후 프롬프트창에 다음과 같이 입력합니다:
   
   ```
   ./task04/guidelines.md에 있는 내용을 따라 프로젝트를 만들어줘.
   ```
...
이 가이드를 따라 Junie와 함께 MVC 패턴, JUnit, JDBC, DB 연결 등을 활용한 전문적인 도서관 관리 시스템을 구축해 보세요. Junie의 강력한 코드 생성 및 프로젝트 구성 기능을 활용하면, 복잡한 프로젝트도 효율적으로 개발할 수 있습니다.